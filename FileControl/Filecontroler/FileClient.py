...
def IfUpload(s, se):
    ...
    if msg.decode('utf-8') == "File Already Exists and Not Changed, Update Done":
        return
    with open(fi, 'rb') as f:
        while True:
            data = f.read(1024)
...
def IfDownload(s, se):
    ...
    print(success)
    if success == 'start':
        print('File Founded, Start Downloading')
    else:
        print('File not Founded, Please Check your File Name')
        return
    root_window = tkinter.Tk()
    fi = filedialog.askdirectory()
    root_window.destroy()
    file_fi = fi + '/' + filename
    with open(file_fi, 'wb') as f:
        while True:
            data = s.recv(1024)
...
def ifCheck(s, se):
    s.send(se[0].encode('utf-8'))
    file_lists = s.recv(1024).decode('utf-8')
    if len(file_lists) > 0:
        print(file_lists)
    else:
        print("当前无下载文件")
